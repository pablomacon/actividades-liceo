...
function iniciarJuego() {
  ...
  resultado.textContent =
    "Juego iniciado. Escribí una combinación y presioná Aceptar.";
...
function aceptarIntento() {
  ...
  resultado.textContent =
    "Leé las luces: verde = lugar correcto, amarillo = está en otro lugar, rojo = no está. Los verdes quedan fijos.";
...
function calcularPistas(intento) {
  ...
  return prueba.map((digito, index) => {
    ...
    if (secreto.includes(digito)) {
      return {
        digito,
        estado: "otra-posicion",
        texto: "Está en otra posición",
      };
    }
...
function pintarLeds(pistas) {
  leds.forEach((led, index) => {
    ...
    if (estado === "correcto") {
      ...
      led.title = "Correcto";
    } else if (estado === "otra-posicion") {
      led.classList.add("led-yellow");
      led.title = "Está en otra posición";
    ...
    tr.innerHTML = `
      ...
      <td>
        <div class="pista-lista">
          ${fila.pistas.map(crearChipPista).join("")}
        </div>
      </td>
    `;
...
function crearChipPista(pista) {
  if (pista.estado === "correcto") {
    return `<span class="pista-chip">🟢 ${pista.digito}: correcto</span>`;
  }

  if (pista.estado === "otra-posicion") {
    return `<span class="pista-chip">🟡 ${pista.digito}: otra posición</span>`;
  }

  return `<span class="pista-chip">🔴 ${pista.digito}: no está</span>`;
}
